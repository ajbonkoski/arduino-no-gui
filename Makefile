...
# set up boards.txt path
BOARDS=$(ARDUINO_DIR)/hardware/arduino/avr/boards.txt
#
# Set up values according to what the IDE uses:
#
DOWNLOAD_RATE = $(shell grep "^$(MODEL)\." $(BOARDS) $(ATTINY_BOARDS) | grep upload.speed | sed 's/.*=//')
MCU = $(shell grep "^$(MODEL)\." $(BOARDS) $(ATTINY_BOARDS) | grep build.mcu | sed 's/.*=//')
F_CPU = $(shell grep "^$(MODEL)\." $(BOARDS) $(ATTINY_BOARDS) | grep build.f_cpu | sed 's/.*=//')
...
# $(ARDUINO_DIR)/hardware/arduino/avr/variants/standard/pins_arduino.c
SRC = \
...
list:
	...
	@cat $(BOARDS) $(ATTINY_BOARDS) | grep '.name' | sed 's/\(.*\)\.name=\(.*\)/MODEL=\1			-> \2/'
